...
u32 BattleAI_ChooseMoveOrAction(u32 battler)
{
    ...
    if (!IsDoubleBattle())
        ...
        ret = ChooseMoveOrAction_Doubles(battler, AI_DATA->targetingCase);
...
static void CalcBattlerAiMovesData(struct AiLogicData *aiData, u32 battlerAtk, u32 battlerDef, u32 weather)
{
    ...
    for (moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
    {
        ...
        if(AI_GetMoveEffectiveness(move, battlerAtk, battlerDef) <= Q_4_12(1.0) && aiData->abilities[battlerDef] == ABILITY_WONDER_GUARD && !IsMoldBreakerTypeAbility(battlerAtk, aiData->abilities[battlerAtk])){
                aiData->simulatedDmg[battlerAtk][battlerDef][moveIndex].expected = 0;
            }
...
static s32 AI_TryToFaint(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    else if ((CanIndexMoveFaintTarget(battlerAtk, battlerDef, movesetIndex, 0) && GetBattleMovePriority(battlerAtk, move) > 0) 
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    case EFFECT_AURORA_VEIL:
        ...
        {
            if(!HasMoveEffect(battlerDef, EFFECT_BRICK_BREAK) && !HasMoveEffect(battlerDef, EFFECT_DEFOG))
                ADJUST_SCORE(WEAK_EFFECT);
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || HasMoveEffect(battlerAtk, EFFECT_SLEEP_TALK)
              || HasMoveEffect(battlerAtk, EFFECT_SNORE)
