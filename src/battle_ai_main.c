...
static u32 Ai_SetMoveAccuracy(struct AiLogicData *aiData, u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    (abilityAtk == ABILITY_FATAL_PRECISION && CalcTypeEffectivenessMultiplier(move, gMovesInfo[move].type, battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], FALSE) >= UQ_4_12(2.0)) || gMovesInfo[move].accuracy == 0) // Moves with accuracy 0 or no guard ability always hit.
        accuracy = 100;
    else
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_RAIN_DANCE:
            ...
            break;
        case EFFECT_HAIL:
            if (weather & (B_WEATHER_HEAVY_HAIL | B_WEATHER_PRIMAL_ANY)
             || IsMoveEffectWeather(aiData->partnerMove))
                ADJUST_SCORE(-8);
            else if (weather & B_WEATHER_SNOW_NORMAL)
                ADJUST_SCORE(-2); // mainly to prevent looping between hail and snow
            break;
        case EFFECT_SNOWSCAPE:
            if (weather & (B_WEATHER_SNOW_NORMAL | B_WEATHER_PRIMAL_ANY)
             || IsMoveEffectWeather(aiData->partnerMove))
                ADJUST_SCORE(-8);
            else if (weather & B_WEATHER_HEAVY_HAIL)
                ADJUST_SCORE(-2); // mainly to prevent looping between hail and snow
            break;
        case EFFECT_ATTRACT:
        ...
        case EFFECT_MOONLIGHT:
            if (AI_BattlerAtMaxHp(battlerAtk))
                ...
                ADJUST_SCORE(-1); //No point in healing, but should at least do it if nothing better
            break;
        case EFFECT_PURIFY:
    ...
    if (HOLD_EFFECT_CHOICE(aiData->holdEffects[battlerAtk]) && IsBattlerItemEnabled(battlerAtk))
    {
        ...
        && !(moveEffect == EFFECT_AURORA_VEIL && (AI_GetWeather() & (B_WEATHER_SNOW_NORMAL | B_WEATHER_HEAVY_HAIL))))
            ADJUST_SCORE(-30);
