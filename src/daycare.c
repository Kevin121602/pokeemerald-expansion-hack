...
void CreateEgg(struct Pokemon *mon, u16 species, bool8 setHotSpringsLocation)
{
    ...
    SetMonData(mon, MON_DATA_PP_BONUSES, &gSpeciesInfo[species].eggCycles);
...
static bool8 TryProduceOrHatchEgg(struct DayCare *daycare)
{
    ...
    // Try to hatch Egg
    daycare->stepCounter++;
    if (((P_EGG_CYCLE_LENGTH <= GEN_3 || P_EGG_CYCLE_LENGTH == GEN_7) && daycare->stepCounter >= 50)
     || (P_EGG_CYCLE_LENGTH == GEN_4 && daycare->stepCounter >= 50)
     || ((P_EGG_CYCLE_LENGTH == GEN_5 || P_EGG_CYCLE_LENGTH == GEN_6) && daycare->stepCounter >= 50)
     || (P_EGG_CYCLE_LENGTH >= GEN_8 && daycare->stepCounter >= 50))
    {
        u32 eggCycles = 0;
        ...
        for (i = 0; i < gPlayerPartyCount; i++)
        {
            ...
            eggCycles = GetMonData(&gPlayerParty[i], MON_DATA_PP_BONUSES);
            if (eggCycles != 0)
            {
                if (eggCycles >= toSub)
                    eggCycles -= toSub;
                ...
                SetMonData(&gPlayerParty[i], MON_DATA_PP_BONUSES, &eggCycles);
            }
            else
            {
