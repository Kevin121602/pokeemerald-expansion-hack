...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr, u8 type);
...
static bool32 AccuracyCalcHelper(u32 move, u32 battler, const u8 *failInstr)
{
    u32 effect = FALSE;
    ...
    {
        if (!JumpIfMoveFailed(7, move, failInstr))
    ...
    else if (gBattleStruct->battlerState[gBattlerTarget].pursuitTarget & (1u << gBattlerTarget))
    {
        JumpIfMoveFailed(7, move, failInstr);
        return TRUE;
    }
    ...
    if (HasWeatherEffect())
    {
        if ((moveEffect == EFFECT_THUNDER || moveEffect == EFFECT_RAIN_ALWAYS_HIT)
            ...
            effect = TRUE;
        else if ((gBattleWeather & B_WEATHER_SNOW) && moveEffect == EFFECT_BLIZZARD)
            effect = TRUE;
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    else
    {
        ...
        for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
            ...
            numTargets++;
            if (JumpIfMoveAffectedByProtect(move, battlerDef, FALSE, failInstr) || AccuracyCalcHelper(move, battlerDef, failInstr))
                continue;
        ...
        else
        {
            ...
            case MOVE_EFFECT_EVS_PLUS_1:
                if (NoAliveMonsForEitherParty()
                  || ChangeStatBuffs(SET_STAT_BUFF_VALUE(1),
                                    ...
                                    affectsUser | STAT_CHANGE_UPDATE_MOVE_EFFECT, 0, moveType) == STAT_CHANGE_DIDNT_WORK)
                {
                    gBattlescriptCurrInstr++;
                }
                else
            ...
            case MOVE_EFFECT_EVS_MINUS_1:
                ...
                if (ChangeStatBuffs(SET_STAT_BUFF_VALUE(1) | STAT_BUFF_NEGATIVE,
                                    ...
                                    flags, gBattlescriptCurrInstr + 1, moveType) == STAT_CHANGE_DIDNT_WORK)
                {
            ...
            case MOVE_EFFECT_EVS_MINUS_2:
                ...
                if (ChangeStatBuffs(SET_STAT_BUFF_VALUE(2) | STAT_BUFF_NEGATIVE,
                                    ...
                                    flags | STAT_CHANGE_UPDATE_MOVE_EFFECT, gBattlescriptCurrInstr + 1, moveType) == STAT_CHANGE_DIDNT_WORK)
                {
                    if (!mirrorArmorReflected)
                        gBattlescriptCurrInstr++;
                }
            ...
            case MOVE_EFFECT_FEINT:
                if (IS_BATTLER_PROTECTED(gBattlerTarget) || IsBattlerSideProtected(gBattlerTarget))
                {
                    ...
                    gProtectStructs[gBattlerTarget].flowerShielded = FALSE;
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
            ...
            case MOVE_EFFECT_HAIL:
            {
                ...
                switch (gBattleScripting.moveEffect)
                {
                    ...
                    case MOVE_EFFECT_SANDSTORM:
                        ...
                        break;
                    case MOVE_EFFECT_HAIL:
                        weather = BATTLE_WEATHER_HEAVY_HAIL;
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                ...
                levelCap = GetIndividualLevelCap(GetMonData(&gPlayerParty[i], MON_DATA_LEVEL_CAP));
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                {      
                    ...
                    if (wasSentOut)
                        gBattleStruct->battlerExpReward = GetSoftLevelCapExpValue(gPlayerParty[*expMonId].level, GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL_CAP), gBattleStruct->expValue);
                    else
                        gBattleStruct->battlerExpReward = 0;
                    ...
                    {
                        gBattleStruct->battlerExpReward += GetSoftLevelCapExpValue(gPlayerParty[*expMonId].level, GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL_CAP), gBattleStruct->expShareExpValue);
                    }
                    ...
                    if (B_EXP_CAP_TYPE == EXP_CAP_HARD && gBattleStruct->battlerExpReward != 0)
                    {
                        ...
                        u32 levelCap = GetIndividualLevelCap(GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL_CAP));

                        if (GetMonData(&gPlayerParty[*expMonId], MON_DATA_LEVEL) >= levelCap)
                            gBattleStruct->battlerExpReward = 0;
                        else if (gExperienceTables[growthRate][(levelCap+1)] < currentExp + gBattleStruct->battlerExpReward)
                            gBattleStruct->battlerExpReward = gExperienceTables[growthRate][(levelCap+1)] - currentExp;
                    }
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                ...
                else if (gProtectStructs[gBattlerTarget].flowerShielded)
                {
                    gProtectStructs[gBattlerAttacker].touchedProtectLike = FALSE;
                    i = gBattlerAttacker;
                    gBattlerAttacker = gBattlerTarget;
...
static void RemoveAllWeather(void)
{
    ...
    else if(gBattleWeather & B_WEATHER_HEAVY_HAIL)
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_CHECK_IF_SOOTHING_STEAM_HEALS:
    {
        ...
        {
            if(def > spDef)
                gBattleStruct->moveDamage[battler] = def / 12;
            else
                gBattleStruct->moveDamage[battler] = spDef / 12;
            if (gBattleStruct->moveDamage[battler] == 0)
                gBattleStruct->moveDamage[battler] = 1;
            gBattleStruct->moveDamage[battler] *= -1;
    ...
    {
        VARIOUS_ARGS();
        BtlController_EmitPlayFanfareOrBGM(battler, B_COMM_TO_CONTROLLER, MUS_DP_VICTORY_TRAINER, TRUE);
        MarkBattlerForControllerExec(battler);
        break;
    }
    ...
    case VARIOUS_SET_AURORA_VEIL:
    {
        ...
        if (gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_AURORA_VEIL
            || !(HasWeatherEffect() && gBattleWeather & B_WEATHER_SNOW))
        {
            gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
            gBattleCommunication[MULTISTRING_CHOOSER] = 0;
        }
...
static void Cmd_setprotectlike(void)
{
        ...
        || (gCurrentMove == MOVE_QUICK_GUARD && B_QUICK_GUARD != GEN_5))
    {
        if (!GetMoveProtectSide(gCurrentMove)) // Protects one mon only.
        {
            ...
            else if (gCurrentMove == MOVE_DETECT || gCurrentMove == MOVE_PROTECT || gCurrentMove == MOVE_DEFEND_ORDER)
            {
            ...
            {
                gProtectStructs[gBattlerAttacker].flowerShielded = TRUE;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PROTECTED_ITSELF;
            }
...
static void Cmd_setfieldweather(void)
{
    ...
    case BATTLE_WEATHER_SANDSTORM:
        ...
        break;
    case BATTLE_WEATHER_HEAVY_HAIL:
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr, u8 type)
{
    ...
    if ((gFieldStatuses & STATUS_FIELD_RICH_SEDIMENT) && (type == TYPE_ROCK || type == TYPE_STEEL || type == TYPE_GROUND)){
...
static void Cmd_statbuffchange(void)
{
    ...
    if (ChangeStatBuffs(GET_STAT_BUFF_VALUE_WITH_SIGN(gBattleScripting.statChanger),
                        ...
                        flags,
                        failInstr, NULL) == STAT_CHANGE_WORKED)
        gBattlescriptCurrInstr = cmd->nextInstr;
...
void BS_SpectralThiefPrintStats(void)
{
    ...
    for (u32 stat = STAT_ATK; stat < NUM_BATTLE_STATS; stat++)
    {
        if (gBattleStruct->stolenStats[0] & (1u << stat))
        {
            ...
            if (ChangeStatBuffs(GET_STAT_BUFF_VALUE_WITH_SIGN(gBattleScripting.statChanger),
                                stat,
                                MOVE_EFFECT_CERTAIN | MOVE_EFFECT_AFFECTS_USER, 0, NULL) == STAT_CHANGE_WORKED)
            {
                BattleScriptPushCursor();
