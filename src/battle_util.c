...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_FIRE     &&  defAbility == ABILITY_FLASH_FIRE)
         || (moveType == TYPE_GRASS    &&  defAbility == ABILITY_SAP_SIPPER)
         || (moveType == TYPE_GROUND   && (defAbility == ABILITY_LEVITATE
                                       ||  defAbility == ABILITY_EARTH_EATER))
         || (moveType == TYPE_WATER    && (defAbility == ABILITY_WATER_ABSORB
                                       || defAbility == ABILITY_DRY_SKIN
                                       || defAbility == ABILITY_STORM_DRAIN))
         || (moveType == TYPE_ELECTRIC && (defAbility == ABILITY_LIGHTNING_ROD // TODO: Add Gen 3/4 config check
                                       || defAbility == ABILITY_VOLT_ABSORB
                                       || defAbility == ABILITY_MOTOR_DRIVE)))
        {
            mod = UQ_4_12(0.0);
        }
