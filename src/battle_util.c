...
bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag, bool32 viaAbility)
{
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SNOW_WARNING:
            ...
            else if (GetGenConfig(GEN_SNOW_WARNING) < GEN_9 && TryChangeBattleWeather(battler, BATTLE_WEATHER_HEAVY_HAIL, gLastUsedAbility))
            {
...
enum HoldEffect GetBattlerHoldEffectIllusion(u32 battler)
{
    if (gBattleMons[battler].volatiles.embargo)
        return HOLD_EFFECT_NONE;
    if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (IsSideProtected(battlerDef, PROTECT_CRAFTY_SHIELD)
     ...
     && GetMoveEffect(move) != EFFECT_COACHING)
        ...
        isProtected = TRUE;
    else if (gProtectStructs[battlerDef].protected == PROTECT_DEFEND_ORDER && !IsBattleMoveStatus(move))
        isProtected = TRUE;
    else
        isProtected = FALSE;
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_SUPREME_OVERLORD:
        ...
        break;
    case ABILITY_ILLUSION:
        if(gBattleStruct->illusion[battlerAtk].state == ILLUSION_ON && (moveType == TYPE_DARK || moveType == TYPE_GHOST || moveType == TYPE_NORMAL))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    default:
        break;
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    if(IsMoldBreakerTypeAbility(ctx->battlerAtk, ctx->abilityAtk)){
        mod = mod;
    }
    else if ((ctx->moveType == TYPE_FIRE     &&  ctx->abilityDef == ABILITY_FLASH_FIRE)
         || (ctx->moveType == TYPE_GRASS    &&  ctx->abilityDef == ABILITY_SAP_SIPPER)
         || (ctx->moveType == TYPE_GROUND   && (ctx->abilityDef == ABILITY_LEVITATE
                                       ||  ctx->abilityDef == ABILITY_EARTH_EATER))
         || (ctx->moveType == TYPE_WATER    && (ctx->abilityDef == ABILITY_WATER_ABSORB
                                       || ctx->abilityDef == ABILITY_DRY_SKIN
                                       || ctx->abilityDef == ABILITY_STORM_DRAIN))
         || (ctx->moveType == TYPE_ELECTRIC && (ctx->abilityDef == ABILITY_LIGHTNING_ROD // TODO: Add Gen 3/4 config check
                                       || ctx->abilityDef == ABILITY_VOLT_ABSORB
                                       || ctx->abilityDef == ABILITY_MOTOR_DRIVE)))
        {
            mod = UQ_4_12(0.0);
        }
...
u16 GetPartyMonFormChangeTargetSpecies(struct BattlePokemon battleMon, enum FormChanges method)
{
    u32 i;
    u32 species = battleMon.species;
    u32 targetSpecies = species;
    const struct FormChange *formChanges = GetSpeciesFormChanges(species);
    //struct Pokemon *mon = GetBattlerMon(battler);
    u16 heldItem = battleMon.item;
    ...
    for (i = 0; formChanges != NULL && formChanges[i].method != FORM_CHANGE_TERMINATOR; i++)
    {
        if (method == formChanges[i].method && species != formChanges[i].targetSpecies)
        {
            ...
            case FORM_CHANGE_BATTLE_MEGA_EVOLUTION_MOVE:
                if (battleMon.moves[0] == formChanges[i].param1
                    || battleMon.moves[1] == formChanges[i].param1
                    || battleMon.moves[2] == formChanges[i].param1
                    || battleMon.moves[3] == formChanges[i].param1)
                    targetSpecies = formChanges[i].targetSpecies;
                break;
            case FORM_CHANGE_BATTLE_SWITCH:
                if (formChanges[i].param1 == battleMon.ability || formChanges[i].param1 == ABILITY_NONE)
                    targetSpecies = formChanges[i].targetSpecies;
                break;
            case FORM_CHANGE_BATTLE_HP_PERCENT:
                if (formChanges[i].param1 == battleMon.ability)
                {
                    // We multiply by 100 to make sure that integer division doesn't mess with the health check.
                    u32 hpCheck = battleMon.hp * 100 * 100 / battleMon.maxHP;
            ...
            case FORM_CHANGE_BATTLE_WEATHER:
                ...
                if (formChanges[i].param2
                    && battleMon.ability != formChanges[i].param2
            ...
            case FORM_CHANGE_STATUS:
                if (battleMon.status1 & formChanges[i].param1)
                    targetSpecies = formChanges[i].targetSpecies;
                break;
            ...
            case FORM_CHANGE_BATTLE_AFTER_MOVE:
                if (formChanges[i].param1 == gCurrentMove
                    && (formChanges[i].param2 == ABILITY_NONE || formChanges[i].param2 == battleMon.ability))
                    targetSpecies = formChanges[i].targetSpecies;
                break;
...
// Returns original species if no form change is possible
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    if (!effect && HasWeatherEffect())
    {
        if (MoveAlwaysHitsInRain(move) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_RAIN))
            effect = TRUE;
        else if ((gBattleWeather & B_WEATHER_SNOW) && MoveAlwaysHitsInHailSnow(move))
            effect = TRUE;
...
static u32 GetSleepTalkMove(void)
{
    ...
    unusableMovesBits = CheckMoveLimitations(gBattlerAttacker, unusableMovesBits, ~(MOVE_LIMITATION_PP | MOVE_LIMITATION_CHOICE_ITEM), FALSE);
