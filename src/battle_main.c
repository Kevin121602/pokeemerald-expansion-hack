        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!IsBattleControllerActiveOrPendingSyncAnywhere(battler))
            {
                ...
                case B_ACTION_BATTLE_INFO:
                    BtlController_ShowBattleInfo(battler, B_COMM_TO_CONTROLLER);
                    MarkBattlerForControllerExec(battler);
                }
...
u32 GetBattlerTotalSpeedStat(u32 battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if (ability == ABILITY_FORECAST && (gBattleWeather & (B_WEATHER_SANDSTORM | B_WEATHER_SNOW | B_WEATHER_RAIN | B_WEATHER_SUN)))
            speed *= 1.5;
    }
...
s32 GetBattleMovePriority(u32 battler, enum Ability ability, u32 move)
{
    ...
    else if (ability == ABILITY_MYCELIUM_MIGHT && IsBattleMoveStatus(move))
    {
        priority--;
    }
    else if (ability == ABILITY_RUN_AWAY && (gMovesInfo[move].effect == EFFECT_HIT_ESCAPE || gMovesInfo[move].effect == EFFECT_PARTING_SHOT || gMovesInfo[move].effect == EFFECT_BATON_PASS))
    {
        priority++;
    }
...
static void TryChangingTurnOrderEffects(struct BattleContext *ctx, u32 *quickClawRandom, u32 *quickDrawRandom)
{
    ...
    if (!gProtectStructs[battler1].quickDraw
     ...
     || (holdEffectBattler1 == HOLD_EFFECT_CUSTAP_BERRY && HasEnoughHpToEatBerry(battler1, ability1, 4, gBattleMons[battler1].item) && !IsUnnerveBlocked(battler1, gBattleMons[battler1].item))))
    ...
    if (!gProtectStructs[battler2].quickDraw
     ...
     || (holdEffectBattler2 == HOLD_EFFECT_CUSTAP_BERRY && HasEnoughHpToEatBerry(battler2, ability2, 4, gBattleMons[battler2].item) && !IsUnnerveBlocked(battler2, gBattleMons[battler2].item))))
...
static void HandleEndTurn_FinishBattle(void)
{
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            if(GetAilmentFromStatus(GetMonData(&gPlayerParty[i], MON_DATA_STATUS)) == AILMENT_SLP){
                recalcSleep = STATUS1_SLEEP_TURN(1 + RandomUniform(RNG_SLEEP_TURNS, 1, 3));
                SetMonData(&gPlayerParty[i], MON_DATA_STATUS, &recalcSleep);
            }
