...
void ItemUseOutOfBattle_GourdSplicer(u8 taskId)
{
    if (!gTasks[taskId].tUsingRegisteredKeyItem)
    {
        gItemUseCB = ItemUseCB_GourdSplicer;
        gTasks[taskId].data[0] = TRUE;
        SetUpItemUseOnFieldCallback(taskId);
    }
    else
...
void ItemUseOutOfBattle_RainbowNectar(u8 taskId)
{
    if (!gTasks[taskId].tUsingRegisteredKeyItem)
    {
        gItemUseCB = ItemUseCB_RainbowNectar;
        gTasks[taskId].data[0] = TRUE;
        SetUpItemUseOnFieldCallback(taskId);
    }
    else
