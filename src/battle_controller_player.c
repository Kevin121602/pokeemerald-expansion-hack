...
static void MoveSelectionDisplayMoveType(u32 battler)
{
    ...
    if(gBattleMons[battler].ability == ABILITY_REFRIGERATE && gMovesInfo[moveInfo->moves[gMoveSelectionCursor[battler]]].type == TYPE_NORMAL){
        type = TYPE_ICE;
    }

    if(gBattleMons[battler].ability == ABILITY_AERILATE && gMovesInfo[moveInfo->moves[gMoveSelectionCursor[battler]]].type == TYPE_NORMAL){
        type = TYPE_FLYING;
    }

    if(gBattleMons[battler].ability == ABILITY_PIXILATE && gMovesInfo[moveInfo->moves[gMoveSelectionCursor[battler]]].type == TYPE_NORMAL){
        type = TYPE_FAIRY;
    }

    if(gBattleMons[battler].ability == ABILITY_NORMALIZE){
        type = TYPE_NORMAL;
    }
