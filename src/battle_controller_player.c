...
static u16 GetPrevBall(u16 ballId)
{
    ...
    for (i = 0; i < gBagPockets[POCKET_GEMS].capacity; i++)
    {
        if (ballId == GetBagItemId(POCKET_GEMS, i))
        {
            if (i <= 0)
            {
                for (j = gBagPockets[POCKET_GEMS].capacity - 1; j >= 0; j--)
                {
                    ballPrev = GetBagItemId(POCKET_GEMS, j);
    ...
    return GetBagItemId(POCKET_GEMS, i);
}
...
static u32 GetNextBall(u32 ballId)
{
    ...
    {
        if (ballId == GetBagItemId(POCKET_GEMS, i-1))
        {
            ballNext = GetBagItemId(POCKET_GEMS, i);
            break;
        }
    }
    if (ballNext == ITEM_NONE)
        return GetBagItemId(POCKET_GEMS, 0); // Zeroth slot
    else
        return ballNext;
}
