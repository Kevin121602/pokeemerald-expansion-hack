...
u32 GetBestDmgMoveFromBattler(u32 battlerAtk, u32 battlerDef)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (gBattleMons[battlerAtk].moves[i] != MOVE_NONE && gBattleMons[battlerAtk].moves[i] != MOVE_UNAVAILABLE && !(unusable & (1u << i))
...
bool32 CanAIFaintTarget(u32 battlerAtk, u32 battlerDef, u32 numHits)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (moves[i] != MOVE_NONE && moves[i] != MOVE_UNAVAILABLE && gMovesInfo[moves[i]].effect != EFFECT_EXPLOSION && !(moveLimitations & (1u << i)))
        {
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if ((weather & B_WEATHER_RAIN) && effect == EFFECT_THUNDER)
        return TRUE;
    if ((weather & B_WEATHER_SNOW) && effect == EFFECT_BLIZZARD)
        return TRUE;
...
bool32 ShouldRecover(u32 battlerAtk, u32 battlerDef, u32 move, u32 healPercent)
{
    ...
    if (AI_DATA->hpPercents[battlerAtk] < 75){
        ...
        {
            if (moves[i] != MOVE_NONE && moves[i] != MOVE_UNAVAILABLE && !(unusable & 1u << i)
                && AI_CalcDamage(moves[i], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected >= healAmount)
            {
                return FALSE;
            }
        }
...
bool32 HasPriorityMove(u32 battler){
    ...
    for(i = 0; i < MAX_MON_MOVES; i++){
        if(GetBattleMovePriority(battler, moves[i]) > 0)
            return TRUE;
    }
...
bool32 ShouldBellyDrum(u32 battlerAtk, u32 battlerDef){
    ...
    u32 bestMoveDmg = AI_CalcDamage(bestMove, battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
...
static u32 IncreaseStatUpScoreInternal(u32 battlerAtk, u32 battlerDef, u32 statId, bool32 considerContrary)
{
    ...
    for(i = 0; i < MAX_MON_MOVES; i++){
        ...
        if(gMovesInfo[moves[i]].category == DAMAGE_CATEGORY_PHYSICAL){
            ...
            if((AI_DATA->simulatedDmg[battlerDef][battlerAtk][i].expected > ignoreBoostsDmg) && (gMovesInfo[moves[i]].ignoresTargetDefenseEvasionStages || sCriticalHitOdds[CalcCritChanceStage(battlerDef, battlerAtk, moves[i], FALSE, AI_DATA->abilities[battlerDef], AI_DATA->abilities[battlerAtk], AI_DATA->holdEffects[battlerDef])] == 1 || CalcCritChanceStage(battlerDef, battlerAtk, moves[i], FALSE, AI_DATA->abilities[battlerDef], AI_DATA->abilities[battlerAtk], AI_DATA->holdEffects[battlerDef]) == -2)){
                ignoreBoostsDmg = AI_DATA->simulatedDmg[battlerDef][battlerAtk][i].expected;
                bestIgnoreBoostsMove = i;
            }
            if(AI_DATA->simulatedDmg[battlerDef][battlerAtk][i].expected > bestPhysPrioDmg && GetBattleMovePriority(battlerDef, moves[i]) > 0){
            ...
            if(AI_DATA->simulatedDmg[battlerDef][battlerAtk][i].expected > bestSpecialPrioDmg && GetBattleMovePriority(battlerDef, moves[i]) > 0){
    ...
    if(bestPhysicalMove != MAX_MON_MOVES){
        bestPhysicalDmg = AI_CalcDamage(moves[bestPhysicalMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }
    if(bestSpecialMove != MAX_MON_MOVES){
        bestSpecialDmg = AI_CalcDamage(moves[bestSpecialMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }

    if(bestPhysPrioMove != MAX_MON_MOVES){
        bestPhysPrioDmg = AI_CalcDamage(moves[bestPhysPrioMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }
    if(bestSpecialPrioMove != MAX_MON_MOVES){
        bestSpecialPrioDmg = AI_CalcDamage(moves[bestSpecialPrioMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }

    if(bestPhysMultiHitMove != MAX_MON_MOVES){
        bestPhysMultiHitDmg = AI_CalcDamage(moves[bestPhysMultiHitMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }

    if(bestSpecialMultiHitMove != MAX_MON_MOVES){
        bestSpecialMultiHitDmg = AI_CalcDamage(moves[bestSpecialMultiHitMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }

    if(bestIgnoreBoostsMove != MAX_MON_MOVES){
        ignoreBoostsDmg = AI_CalcDamage(moves[bestIgnoreBoostsMove], battlerDef, battlerAtk, &effectiveness, TRUE, AI_GetWeather(), DMG_ROLL_DEFAULT).expected;
    }
...
void IncreasePoisonScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    if (AI_CanPoison(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], move, AI_DATA->partnerMove) && AI_DATA->hpPercents[battlerDef] > 20)
    {
        ...
        if ((HasMoveEffect(battlerAtk, EFFECT_PROTECT) || AI_DATA->abilities[battlerAtk] == ABILITY_MERCILESS) || (IsPowerBasedOnStatus(battlerAtk, EFFECT_DOUBLE_POWER_ON_ARG_STATUS, STATUS1_POISON)))
