...
bool32 PartyMonHasInTactFocusSashSturdy(u32 battlerAtk, u32 battlerDef, u32 move, u32 holdEffect, u32 ability, struct BattlePokemon battleMon, bool32 isPartyMonCheckedForSash)
{
    ...
    else
    {
        ...
        if(holdEffect == HOLD_EFFECT_FOCUS_SASH && AI_BattlerAtMaxHp(battlerAtk)){
            return TRUE;
        }

        if(ability == ABILITY_STURDY && AI_BattlerAtMaxHp(battlerAtk) && !IsMoldBreakerTypeAbility(battlerAtk, battleMon.ability) && !gMovesInfo[move].ignoresTargetAbility){
            return TRUE;
        }
...
bool32 MonHasInTactFocusSashSturdy(u32 battler, u32 opposingBattler, u32 holdEffect, u32 ability, u32 move)
{
    ...
    if(holdEffect == HOLD_EFFECT_FOCUS_SASH && AI_BattlerAtMaxHp(battler)){
        return TRUE;
    }

    if(ability == ABILITY_STURDY && AI_BattlerAtMaxHp(battler) && !IsMoldBreakerTypeAbility(opposingBattler, gBattleMons[opposingBattler].ability) && !gMovesInfo[move].ignoresTargetAbility){
        return TRUE;
    }
...
struct SimulatedDamage AI_CalcDamageSaveBattlers(u32 move, u32 battlerAtk, u32 battlerDef, uq4_12_t *typeEffectiveness, enum AIConsiderGimmick considerGimmickAtk, enum AIConsiderGimmick considerGimmickDef, bool8 isSwitchCalc)
{
    ...
    dmg = AI_CalcDamage(move, battlerAtk, battlerDef, typeEffectiveness, considerGimmickAtk, considerGimmickDef, AI_GetWeather(), isSwitchCalc);
...
struct SimulatedDamage AI_CalcDamage(u32 move, u32 battlerAtk, u32 battlerDef, uq4_12_t *typeEffectiveness, enum AIConsiderGimmick considerGimmickAtk, enum AIConsiderGimmick considerGimmickDef, u32 weather, bool8 isSwitchCalc)
{
    ...
    if (movePower && !IsDamageMoveUnusable(&ctx))
    {
        ...
        if ((aiData->abilities[battlerAtk] == ABILITY_PROTEAN || aiData->abilities[battlerAtk] == ABILITY_LIBERO)
            ...
            && (gBattleMons[battlerAtk].types[0] != ctx.moveType || gBattleMons[battlerAtk].types[1] != ctx.moveType
                || (gBattleMons[battlerAtk].types[2] != ctx.moveType && gBattleMons[battlerAtk].types[2] != TYPE_MYSTERY))
            && move != MOVE_STRUGGLE
        ...
        if(moveEffect == EFFECT_REVENGE && !HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_STATUS) && !isSwitchCalc){
            simDamage.minimum *= 2;
            simDamage.median *= 2;
            simDamage.maximum *= 2;
        }
...
s32 AI_WhoStrikesFirst(u32 battlerAI, u32 battler, u32 aiMoveConsidered, u32 playerMoveConsidered, enum ConsiderPriority considerPriority)
{
    ...
    //s8 playerPriority = GetBattleMovePriority(battler, abilityPlayer, moveConsidered);
...
bool32 CanAIFaintTarget(u32 battlerAtk, u32 battlerDef, u32 numHits)
{
    ...
    for (moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
    {
        if (moves[moveIndex] != MOVE_NONE && !IsMoveUnusable(moveIndex, moves[moveIndex], moveLimitations) && gAiThinkingStruct->score[moveIndex] >= 100)
        {
...
enum Ability AI_DecideKnownAbilityForTurn(u32 battlerId)
{
    ...
    enum Ability knownAbility = GetBattlerAbilityIgnoreMoldBreaker(battlerId); // during ai checking for mold breaker could lead to inaccuracies
    //u32 indexAbility;
...
u32 AI_GetSwitchinWeather(struct BattlePokemon battleMon)
{
    ...
    case ABILITY_SNOW_WARNING:
        return GetConfig(CONFIG_SNOW_WARNING) >= GEN_9 ? B_WEATHER_SNOW_NORMAL : B_WEATHER_HAIL_NORMAL;
    default:
        return gBattleWeather;
    }
}
...
s32 ProtectChecks(u32 battlerAtk, u32 battlerDef, u32 move, u32 predictedMove, s32 *score)
{

    u32 noOfHitsToFaint = NoOfHitsForTargetToFaintBattler(battlerDef, battlerAtk);

    u32 speedBattlerAI, speedBattler;
    u32 holdEffectAI = gAiLogicData->holdEffects[battlerAtk];
    u32 holdEffectPlayer = gAiLogicData->holdEffects[battlerDef];
    u32 abilityAI = gAiLogicData->abilities[battlerAtk];
    ...
    if (IsDoubleBattle()){
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            if((gBattleHistory->usedMoves[battlerAtk][i] == move) && Random() % 100 < 50)
                ADJUST_SCORE_PTR(-30);
        }
    }
...
bool32 CanLowerStat(u32 battlerAtk, u32 battlerDef, struct AiLogicData *aiData, enum Stat stat)
{
    ...
    if (stat == STAT_SPEED)
    {
        return !(AI_IsFaster(battlerAtk, battlerDef, MOVE_NONE, MOVE_NONE, DONT_CONSIDER_PRIORITY));
    }
...
{
    if (AI_IsFaster(battlerAtk, battlerDef, gAiThinkingStruct->moveConsidered, 0, DONT_CONSIDER_PRIORITY)
...
bool32 ShouldLowerAccuracy(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
    ...
    if (defAbility != ABILITY_CONTRARY
      ...
      && (GetConfig(CONFIG_ILLUMINATE_EFFECT) >= GEN_9 && defAbility != ABILITY_ILLUMINATE)
      && gAiLogicData->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
        return TRUE;
    return FALSE;
}
...
bool32 AI_CanPoison(u32 battlerAtk, u32 battlerDef, enum Ability defAbility, u32 move, u32 partnerMove)
{
    if (!CanBePoisoned(battlerAtk, battlerDef, gAiLogicData->abilities[battlerAtk], defAbility)
      ...
      || gAiLogicData->abilities[battlerDef] == ABILITY_MAGIC_GUARD)
        return FALSE;
...
bool32 ShouldRollout(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if(HasMoveWithEffect(battlerDef, EFFECT_PROTECT)){
        return FALSE;
    }
...
bool32 ShouldSetScreen(u32 battlerAtk, u32 battlerDef, enum BattleMoveEffects moveEffect)
{
    ...
    case EFFECT_AURORA_VEIL:
        ...
        if ((AI_GetWeather() & B_WEATHER_SNOW)
...
static u32 GetHPWithBerry(u32 battler){
    ...
    u32 heldItemEffect = GetItemHoldEffect(gBattleMons[battler].item);
    u8 holdEffectParam = GetItemHoldEffectParam(gBattleMons[battler].item);
...
bool32 HasPriorityMove(u32 battler){
    ...
    for(i = 0; i < MAX_MON_MOVES; i++){
        if(GetBattleMovePriority(battler, gAiLogicData->abilities[battler], moves[i]) > 0)
            return TRUE;
    }
...
static u32 IncreaseStatUpScoreInternal(u32 battlerAtk, u32 battlerDef, enum StatChange statId, bool32 considerContrary)
{
    ...
    u32 bestAIDmgOnPlayer = GetBestDmgFromBattler(battlerAtk, battlerDef, AI_ATTACKING);
    ...
    for(i = 0; i < MAX_MON_MOVES; i++){
        ...
        if(gMovesInfo[moves[i]].category == DAMAGE_CATEGORY_PHYSICAL){
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestPhysicalDmg){
                bestPhysicalDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestPhysicalMove = i;
            }
            if((gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > ignoreBoostsDmg) && (gMovesInfo[moves[i]].ignoresTargetDefenseEvasionStages || sCriticalHitOdds[CalcCritChanceStage(battlerDef, battlerAtk, moves[i], FALSE, gAiLogicData->abilities[battlerDef], gAiLogicData->abilities[battlerAtk], gAiLogicData->holdEffects[battlerDef])] == 1 || CalcCritChanceStage(battlerDef, battlerAtk, moves[i], FALSE, gAiLogicData->abilities[battlerDef], gAiLogicData->abilities[battlerAtk], gAiLogicData->holdEffects[battlerDef]) == -2)){
                ignoreBoostsDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestIgnoreBoostsMove = i;
            }
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestPhysPrioDmg && GetBattleMovePriority(battlerDef, abilityPlayer, moves[i]) > 0){
                bestPhysPrioDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestPhysPrioMove = i;
            }
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestPhysMultiHitDmg && (gMovesInfo[moves[i]].effect == EFFECT_MULTI_HIT || gMovesInfo[moves[i]].strikeCount > 1)){
                bestPhysMultiHitDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestPhysMultiHitMove = i;
            }
        } else {
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestSpecialDmg){
                bestSpecialDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestSpecialMove = i;
            } 
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestSpecialPrioDmg && GetBattleMovePriority(battlerDef, abilityPlayer, moves[i]) > 0){
                bestSpecialPrioDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
                bestSpecialPrioMove = i;
            }
            if(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median > bestSpecialMultiHitDmg && (gMovesInfo[moves[i]].effect == EFFECT_MULTI_HIT || gMovesInfo[moves[i]].strikeCount > 1)){
                bestSpecialMultiHitDmg = gAiLogicData->simulatedDmg[battlerDef][battlerAtk][i].median;
    ...
    //u32 hitsToKoShellSmash = GetNoOfHitsToKO(gAiLogicData->simulatedDmg[battlerDef][battlerAtk][bestMove].median, (gBattleMons[battlerAtk].hp/1.5));
    ...
    if(statId == STAT_CHANGE_SHELL_SMASH && holdEffectAI != HOLD_EFFECT_WHITE_HERB){
...
void IncreasePoisonScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    if (AI_CanPoison(battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], move, gAiLogicData->partnerMove) && gAiLogicData->hpPercents[battlerDef] > 20)
    {
        ...
        if ((HasMoveWithEffect(battlerAtk, EFFECT_PROTECT) || gAiLogicData->abilities[battlerAtk] == ABILITY_MERCILESS) || (IsPowerBasedOnStatus(battlerAtk, EFFECT_DOUBLE_POWER_ON_ARG_STATUS, STATUS1_POISON)))
...
{
    if(gAiLogicData->abilities[battlerDef] == ABILITY_GUTS)
        return;
    ...
    if (AI_CanBurn(battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move, gAiLogicData->partnerMove))
    {
        ...
        if (gMovesInfo[GetBestDmgMoveFromBattler(battlerDef, battlerAtk, AI_DEFENDING)].category == DAMAGE_CATEGORY_PHYSICAL && Random() % 100 < 60)
                ADJUST_SCORE_PTR(WEAK_EFFECT);
...
u32 IncreaseFollowMeScore(u32 battlerAtk, u32 battlerDef){
    ...
    if(gMovesInfo[GetBestDmgMoveFromBattler(battlerDef, battlerPartner, AI_DEFENDING)].target == MOVE_TARGET_FOES_AND_ALLY || gMovesInfo[GetBestDmgMoveFromBattler(battlerDef, battlerPartner, AI_DEFENDING)].target == MOVE_TARGET_BOTH)
        return NO_INCREASE;
...
u32 IncreaseMagnetRiseScore(u32 battlerAtk, u32 battlerDef){
    ...
    if(gMovesInfo[GetBestDmgMoveFromBattler(battlerDef, battlerAtk, AI_DEFENDING)].type != TYPE_GROUND || gMovesInfo[GetBestDmgMoveFromBattler(battlerDef, battlerAtk, AI_DEFENDING)].ignoreTypeIfFlyingAndUngrounded)
        return NO_INCREASE;
...
u32 IncreaseStatLoweringScore(u32 battlerAtk, u32 battlerDef, u32 statId, u32 stages){
    ...
    if (gAiLogicData->holdEffects[battlerDef] == HOLD_EFFECT_CLEAR_AMULET
         || gAiLogicData->abilities[battlerDef] == ABILITY_CLEAR_BODY
    ...
    if(statId == STAT_CHANGE_DEF && gAiLogicData->abilities[battlerDef] == ABILITY_BIG_PECKS)
        return NO_INCREASE;

    if(statId == STAT_CHANGE_ACC && (gAiLogicData->abilities[battlerDef] == ABILITY_ILLUMINATE || gAiLogicData->abilities[battlerDef] == ABILITY_KEEN_EYE || gAiLogicData->abilities[battlerDef] == ABILITY_NO_GUARD))
        return NO_INCREASE;
...
bool32 ShouldUseZMove(u32 battlerAtk, u32 battlerDef, u32 chosenMove)
{
    ...
    if (IsViableZMove(battlerAtk, chosenMove))
    {
        ...
        dmg = AI_CalcDamageSaveBattlers(chosenMove, battlerAtk, battlerDef, &effectiveness, NO_GIMMICK, NO_GIMMICK, FALSE);
...
void DecideTerastal(u32 battler)
{
    ...
    for (int i = 0; i < MAX_MON_MOVES; i++)
    {
        if (!IsMoveUnusable(i, aiMoves[i], gAiLogicData->moveLimitations[battler]) && !IsBattleMoveStatus(aiMoves[i]))
            altCalcs.dealtWithoutTera[i] = AI_CalcDamage(aiMoves[i], battler, opposingBattler, &effectiveness, NO_GIMMICK, NO_GIMMICK, AI_GetWeather(), FALSE);
        else
        ...
        {
            altCalcs.takenWithTera[i] = AI_CalcDamage(oppMoves[i], opposingBattler, battler, &effectiveness, USE_GIMMICK, USE_GIMMICK, AI_GetWeather(), FALSE);
...
u32 GetDoublesTargeting(u32 battlerAtk, u32 battlerDef){
    ...
    u32 holdEffectAI = gAiLogicData->holdEffects[battlerAtk];
    u32 holdEffectAIPartner = gAiLogicData->holdEffects[aiPartner];
    u32 abilityAI = gAiLogicData->abilities[battlerAtk];
    u32 abilityAIPartner = gAiLogicData->abilities[aiPartner];

    u32 speedBattler, speedBattlerPartner;
    u32 holdEffect = gAiLogicData->holdEffects[battlerDef];
    u32 holdEffectPartner = gAiLogicData->holdEffects[playerPartner];
    u32 ability = gAiLogicData->abilities[battlerDef];
    u32 abilityPartner = gAiLogicData->abilities[playerPartner];

    speedBattlerAI = GetBattlerTotalSpeedStat(battlerAtk, abilityAI, holdEffectAI);
    speedBattlerAIPartner   = GetBattlerTotalSpeedStat(aiPartner, abilityAIPartner, holdEffectAIPartner);

    speedBattler = GetBattlerTotalSpeedStat(battlerDef, ability, holdEffect);
    speedBattlerPartner   = GetBattlerTotalSpeedStat(playerPartner, abilityPartner, holdEffectPartner);

    bool32 aiHasKillSlot1 = (NoOfHitsForTargetToFaintBattler(battlerAtk, battlerDef) <= 1);
    bool32 aiHasKillSlot2 = (NoOfHitsForTargetToFaintBattler(battlerAtk, playerPartner) <= 1);
    bool32 aiPartnerHasKillSlot1 = (NoOfHitsForTargetToFaintBattler(aiPartner, battlerDef) <= 1);
    bool32 aiPartnerHasKillSlot2 = (NoOfHitsForTargetToFaintBattler(aiPartner, playerPartner) <= 1);
