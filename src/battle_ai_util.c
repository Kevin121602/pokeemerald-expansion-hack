...
static inline void CalcDynamicMoveDamage(struct DamageCalculationData *damageCalcData, u16 *medianDamage, u16 *minimumDamage, u16 *maximumDamage, enum ItemHoldEffect holdEffectAtk, u32 abilityAtk)
{
    ...
    case EFFECT_LEVEL_DAMAGE:
        median = minimum = gBattleMons[damageCalcData->battlerAtk].level * (abilityAtk == ABILITY_PARENTAL_BOND ? 2 : 1);
        if(AI_GetMoveEffectiveness(move, damageCalcData->battlerAtk, damageCalcData->battlerDef) == Q_4_12(0.0)){
                    median = minimum = 0;
        }
        break;
    case EFFECT_PSYWAVE:
    ...
    case EFFECT_FIXED_DAMAGE_ARG:
        median = minimum = GetMoveFixedDamage(move) * (abilityAtk == ABILITY_PARENTAL_BOND ? 2 : 1);
        if(AI_GetMoveEffectiveness(move, damageCalcData->battlerAtk, damageCalcData->battlerDef) == Q_4_12(0.0)){
                    median = minimum = 0;
        }
        break;
    ...
    case EFFECT_ENDEAVOR:
        ...
        median = minimum = max(0, gBattleMons[damageCalcData->battlerDef].hp - gBattleMons[damageCalcData->battlerAtk].hp);
        break;
    case EFFECT_SUPER_FANG:
        median = minimum = (abilityAtk == ABILITY_PARENTAL_BOND
    ...
    case EFFECT_FINAL_GAMBIT:
        median = minimum = gBattleMons[damageCalcData->battlerAtk].hp;
        if(AI_GetMoveEffectiveness(move, damageCalcData->battlerAtk, damageCalcData->battlerDef) == Q_4_12(0.0)){
                    median = minimum = 0;
        }
        break;
...
u32 GetMinNoOfHitsToKOBattler(u32 battlerAtk, u32 battlerDef)
{
    ...
    for(int i = 0; i < MAX_MON_MOVES; i++){
        if(gBattleMons[battlerAtk].moves[i] != MOVE_NONE
            ...
            && !IsBattleMoveStatus(gBattleMons[battlerAtk].moves[i]))
        numHits = GetNoOfHitsToKOBattlerDmg(gAiLogicData->simulatedDmg[battlerAtk][battlerDef][i].median, battlerDef);
        if(numHits < minHits)
            minHits = numHits;
    }
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (gAiLogicData->abilities[battlerAtk] == ABILITY_FATAL_PRECISION && CalcTypeEffectivenessMultiplier(move, gMovesInfo[move].type, battlerAtk, battlerDef, gAiLogicData->abilities[battlerDef], FALSE) >= UQ_4_12(2.0))
        return TRUE;
    ...
    if ((weather & B_WEATHER_RAIN) && MoveAlwaysHitsInRain(move))
        return TRUE;
    if ((weather & B_WEATHER_SNOW) && MoveAlwaysHitsInHailSnow(move))
        return TRUE;
...
bool32 ShouldRecover(u32 battlerAtk, u32 battlerDef, u32 move, u32 healPercent)
{
    ...
    if (gAiLogicData->hpPercents[battlerAtk] < 75){
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            if (moves[i] != MOVE_NONE && moves[i] != MOVE_UNAVAILABLE && !IsMoveUnusable(i, moves[i], moveLimitations)
                && AI_CalcDamage(moves[i], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median >= healAmount)
            {
                return FALSE;
            }
        }
...
bool32 ShouldBellyDrum(u32 battlerAtk, u32 battlerDef){
    ...
    u32 bestMoveDmg = AI_CalcDamage(bestMove, battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
...
static u32 IncreaseStatUpScoreInternal(u32 battlerAtk, u32 battlerDef, enum StatChange statId, bool32 considerContrary)
{
    ...
    if(bestPhysicalMove != MAX_MON_MOVES){
        bestPhysicalDmg = AI_CalcDamage(moves[bestPhysicalMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }
    if(bestSpecialMove != MAX_MON_MOVES){
        bestSpecialDmg = AI_CalcDamage(moves[bestSpecialMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }

    if(bestPhysPrioMove != MAX_MON_MOVES){
        bestPhysPrioDmg = AI_CalcDamage(moves[bestPhysPrioMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }
    if(bestSpecialPrioMove != MAX_MON_MOVES){
        bestSpecialPrioDmg = AI_CalcDamage(moves[bestSpecialPrioMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }

    if(bestPhysMultiHitMove != MAX_MON_MOVES){
        bestPhysMultiHitDmg = AI_CalcDamage(moves[bestPhysMultiHitMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }

    if(bestSpecialMultiHitMove != MAX_MON_MOVES){
        bestSpecialMultiHitDmg = AI_CalcDamage(moves[bestSpecialMultiHitMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }

    if(bestIgnoreBoostsMove != MAX_MON_MOVES){
        ignoreBoostsDmg = AI_CalcDamage(moves[bestIgnoreBoostsMove], battlerDef, battlerAtk, &effectiveness, FALSE, AI_GetWeather()).median;
    }
