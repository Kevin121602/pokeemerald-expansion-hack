...
#define B_SCALED_EXP                FALSE // In Gen5 and Gen7+, experience is weighted by level difference.
#define B_UNEVOLVED_EXP_MULTIPLIER  FALSE // In Gen6+, if the Pokémon is at or past the level where it would be able to evolve, but it has not, it gets a ~1.2 multiplier to EXP gain. Only applies to Pokémon with EVO_LEVEL method.
#define B_LEVEL_UP_NOTIFICATION     FALSE // In Gen9+, if the Pokémon gets enough experience to level up multiple times, the message is only displayed once.
...
#define B_CHARGE_SPDEF_RAISE        FALSE // In Gen5+, Charge raises the user's Special Defense by 1 stage.
...
#define B_DEFOG_EFFECT_CLEARING     GEN_6 // In Gen6+, Defog clears Spikes, Toxic Spikes, Stealth Rock and Sticky Web from both sides. In Gen8+, Defog also clears active Terrain.
...
#define B_MOODY_ACC_EVASION         GEN_7 // In Gen8, Moody CANNOT raise Accuracy and Evasion anymore.
...
#define B_RESTORE_HELD_BATTLE_ITEMS GEN_4 // In Gen9, all non-berry items are restored after battle.
...
#define B_VAR_NO_BAG_USE         NO_BAG_AGAINST_TRAINER     // If 1, the ability to use the bag in battle is disabled in trainer battles. If 2, it is also disabled in wild battles.
...
// Catching settings
#define B_SEMI_INVULNERABLE_CATCH       FALSE // In Gen4+, you cannot throw a ball against a Pokemon that is in a semi-invulnerable state (dig/fly/etc)
...
#define B_CATCH_SWAP_INTO_PARTY     FALSE // In Gen 7+, the option to swap the caught wild mon to the party will appear, allowing you to send a different mon to the box.
...
#define B_WILD_NATURAL_ENEMIES          FALSE       // If set to TRUE, certain wild mon species will attack other species when partnered in double wild battles (eg. Zangoose vs Seviper)
